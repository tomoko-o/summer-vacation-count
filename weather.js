...
fetch(url)
  ...
  .then(function (weather) {
    ...
    const weatherCode = weather[0].timeSeries[0].areas[0].weatherCodes[0];
    // console.log(weather[0].timeSeries[0].areas[0].weatherCodes);
    // console.log(JSON.stringify(weather, null, 8));
    // console.log(weather);
    const timeDefines = weather[0].timeSeries[0].timeDefines;
    const startCount = weather[1].timeSeries[0].timeDefines.indexOf(timeDefines[1]);
    // console.log(startCount);
    document.getElementById('weather_img').src = "https://www.jma.go.jp/bosai/forecast/img/" + weatherCodeList[weatherCode][0];
    document.getElementById('weather_text').textContent = weatherCodeList[weatherCode][2];
    ...
    let tempsMax;
    if (isTodaysData) {
      tempsMin = temps[0] === temps[1] ? temps[2] : temps[0];
      tempsMax = temps[1];
    } else {
      tempsMin = ("--", temps[0]);
      tempsMax = ("--", temps[1]);
    ...
    document.getElementById('temps_min').textContent = `${tempsMin}℃`;
    document.getElementById('temps_max').textContent = `${tempsMax}℃`;
